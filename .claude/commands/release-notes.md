...
git log $range --pretty=format:"%H|%s" | ForEach-Object {
    ...
    # Linear 이슈 ID 추출 (예: MIC-12)
    foreach ($im in [regex]::Matches($msg, 'MIC-\d+')) {
        if (-not $issueIds.Contains($im.Value)) { $issueIds.Add($im.Value) }
    }
...
Write-Host "ISSUE_IDS:$($issueIds -join ',')"
```
...
- "포함된 이슈" 섹션은 PowerShell 출력의 `ISSUE_IDS`에 식별자가 있을 때만 포함한다. 각 식별자는 `mcp__linear__get_issue`로 조회해 제목과 URL을 채운다.
...
### 4-B. 포함된 이슈에 릴리즈 코멘트 등록 (Linear)

`ISSUE_IDS`로 추출된 식별자가 있는 경우에만 진행한다. 없으면 건너뛴다.

상태 전환(`Done` 등)은 GitHub-Linear 자동 연동(PR 머지 시 트리거)에 맡긴다 — 릴리즈 시점엔 이미 자동으로 Done 처리됐을 가능성이 높아 수동 전환은 중복이다. 대신 자동화가 캡처하지 못하는 "어느 버전에 실릴렸는지" 정보를 코멘트로 남긴다.

```
이번 릴리즈에 포함된 이슈 {N}건에 "v{버전}에 포함되어 릴리즈됨" 코멘트를 남길까요? (y/n)
```
사용자가 승인하면 추출된 각 식별자에 대해 `mcp__linear__save_comment(id={식별자}, body="v{버전}에 포함되어 릴리즈되었습니다.")`를 호출하고, 결과를 표로 출력한다.
