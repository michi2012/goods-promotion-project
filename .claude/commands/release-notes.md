...
### 4-B. 포함된 이슈 상태 전환 (Linear)

`ISSUE_IDS`로 추출된 식별자가 있는 경우에만 진행한다. 없으면 건너뛴다.

```
이번 릴리즈에 포함된 이슈 {N}건을 Linear에서 "Done" 상태로 전환할까요? (y/n)
```
사용자가 승인하면 추출된 각 식별자에 대해 `mcp__linear__save_issue(id={식별자}, state="Done")`를 호출하고, 결과를 표로 출력한다.
