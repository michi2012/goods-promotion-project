...
- **[필수] 스키마 동기화 (MCP):** 엔티티/쿼리 작성 전 반드시 아래를 수행한다. DB 구조를 상상해서 작성하는 것을 엄격히 금지한다.
    1. `list_tables` (또는 해당 MCP가 지원하는 목록 조회 툴)로 테이블 존재 여부를 확인한다.
    2. 제약조건과 인덱스 현황을 한 번에 파악하기 위해 쿼리 실행 툴을 사용하여 `SHOW CREATE TABLE {table}`을 실행하고 DDL을 읽어라. (만약 권한/툴 문제로 실패한다면 `describe_table`과 `SHOW INDEX FROM {table}`을 병행 실행하라.)

**MCP 미연결 시:** DB 툴을 사용할 수 없는 경우, 코딩 시작 전 반드시 사용자에게 해당 테이블의 DDL(SHOW CREATE TABLE 결과)을 요청하라. 스키마를 추측하지 마라.

- **[필수] 쿼리 실행 계획 검증 (EXPLAIN):** Repository 쿼리 추가 시 반드시 `EXPLAIN`을 선행 실행하고 다음 룰을 따른다.
    1. **자체 튜닝 우선:** `type = ALL` (풀 스캔)이 발생하면 즉시 보고하지 말고, 네가 먼저 쿼리 튜닝(조건절 변경, JOIN 방식 변경 등)을 1회 이상 시도하라.
    2. **임계치 기반 패스:** 튜닝 후에도 풀 스캔이 불가피하지만, 현재 rows가 1,000건 미만이고 해당 테이블의 향후 최대 예상 규모도 작다고 판단되거나, 전체 데이터를 의도적으로 읽어야 하는 상황이라면 **보고 없이 작업을 계속 진행한다.**
    3. **인덱스 제안 및 스크립트화 (직접 실행 금지):** `rows`가 1,000건 이상이고 인덱스가 반드시 필요하다고 판단될 경우, MCP를 통해 DB에 직접 인덱스를 추가하려 하지 마라. 대신 `/docs` (또는 프로젝트 구조에 맞는 경로)에 `V{날짜}__add_idx_{테이블명}.sql` 형태의 DDL 파일을 생성한 뒤, 나에게 "인덱스 마이그레이션 파일을 생성했으니 검토 후 적용해달라"고만 보고하라.

[경고] 읽기 전용 원칙: 너에게 부여된 DB 툴은 오직 스키마 분석(SHOW CREATE TABLE)과 쿼리 성능 분석(EXPLAIN)을 위한 '읽기 전용'이다. INSERT, UPDATE, DELETE, DROP, ALTER 등의 DML/DDL 쿼리를 절대 실행하지 마라.
...
- [ ] 코드를 짜기 전 툴을 이용해 실제 DB 스키마(제약조건, 인덱스)를 100% 동기화하여 확인했는가?
- [ ] Repository에 쿼리 추가 시, EXPLAIN을 돌려보고 튜닝 후에도 풀 스캔이 남고 rows ≥ 1,000이면 인덱스 마이그레이션 파일을 생성해 보고했는가?