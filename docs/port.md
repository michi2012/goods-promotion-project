...
[serverA: Saga Orchestrator]
 ...
 → 202 Accepted 즉시 반환
   (serverB 주문 상태 api 폴링 — 1-2s 후 시작
    NOT_FOUND 5s 이상 지속 시 주문 실패로 간주 / PENDING 확인 시 Saga 대기 및 폴링 지속 (PAID / FAILED / EXPIRED))
   │ purchase_events (직접 발행, 비동기 — 실패 시 콜백 롤백)
   ▼
...
| `incident` | 장애 발생 시 사실 수집 · 관련 커밋 조회 → RCA 문서 자동 생성 (`docs/incidents/` 저장) |
| `db-migration` | 엔티티 변경 감지 → Flyway SQL 초안 제안 + 기존 마이그레이션 파일 🔴/🟡/🟢 위험도 자동 분석 |
...
Flyway로 DB 스키마를 버전 관리하고(JPA `ddl-auto` 제거), `/db-migration` 스킬이 머지 전 DDL 위험도를 자동 분석한다. 엔티티 변경을 감지해 다음 Flyway 버전 번호가 붙은 SQL 초안을 제안하고, 기존 마이그레이션 파일의 `DROP COLUMN`·`NOT NULL DEFAULT 없음`·`RENAME TABLE` 등 위험 패턴을 🔴/🟡/🟢 3단계로 분류해 체크리스트와 함께 출력한다. 머지 전 담당자가 위험도를 인지하고 롤백 계획을 수립하도록 강제하는 운영 안전망으로 활용한다.
...
- **운영 자동화**: `/incident`로 장애 발생 시 사실 수집·관련 커밋 조회·RCA 문서 생성을 단일 커맨드로 자동화해 장애 중 수작업 제거. `/db-migration`으로 머지 전 DDL 위험 패턴(`DROP COLUMN`·`NOT NULL DEFAULT 없음`·`RENAME TABLE`)을 🔴/🟡/🟢 3단계로 자동 분류하고 Flyway 버전 번호가 붙은 SQL 초안을 제안해 마이그레이션 실수 방지
